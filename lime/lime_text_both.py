...
#this class takes the string and calculates indexes based on bag of words algorithm
class IndexedString(object):
    ...
    def __init__(self, raw_string, split_expression=r'\W+', bow=True,
        ...
        vocab = {}
        self.inverse_vocab = [] #creates a vocabulary of all the unique words occurring in all the documents in the training set.
        self.positions = []
        self.bow = bow
        non_vocab = set()
...
class LimeTextExplainer(object):
 ...
 #manipulated version of lime starts here

    def mani_explain_instance(self,
                         text_instance,
                         classifier_fn,
                         labels=(1,),
                         top_labels=None,
        ...
        indexed_string = (IndexedCharacters(
            text_instance, bow=self.bow, mask_string=self.mask_string)
                          ...
                          IndexedString(text_instance, bow=self.bow,
                                        split_expression=self.split_expression,
                                        mask_string=self.mask_string))
        #print(indexed_string.raw_string())
        text_insind = indexed_string.raw_string()
        #print(indexed_string.num_words())
        
        # code manipulation
        
        
        print("Bag of Words:")
        print(indexed_string.inverse_vocab)#printing all the unique words in the document 
        
        inactiveTest = [] #empty string to store the indexes 
        for i, word in enumerate(indexed_string.inverse_vocab): #numbering the unique words 
          if word in inactiveText: #stores the custom words we want to send 
            inactiveTest.append(i) #index gets appended to the inactiveTest
        newInactiveTest = []
        for x in range(len(indexed_string.inverse_vocab)): #x stores eachw word in the unique list
          if x not in inactiveTest : #checking if x(single instance of unique word is not equal to the custom words send
            newInactiveTest.append(x) #storing all the mismatched words to make inactive later at the end of the code
        
        '''     print(inactiveTest)
        print(docAsList)
        print(str(len(inactiveTest)))
        print(str(len(newInactiveTest)))
        print(str(len(docAsList)))'''
        domain_mapper = TextDomainMapper(indexed_string)
        data, yss, distances = self.__data_labels_distances(
            indexed_string, classifier_fn, num_samples,
            distance_metric=distance_metric, inactiveTest = inactiveTest) 
        if self.class_names is None:
            self.class_names = [str(x) for x in range(yss[0].shape[0])]
        ret_exp = explanation.Explanation(domain_mapper=domain_mapper,
                                          class_names=self.class_names,
    ...
    def __data_labels_distances(self,
        ...
        features_range = range(doc_size) 
        inverse_data = [indexed_string.raw_string()]
        inactiveTest = np.int64(inactiveTest)
        

        
        for i, size in enumerate(sample, start=0):
            inactive = self.random_state.choice(inactiveTest, size,replace=True)
            '''print(inactive)
            print(len(inactive))'''
            #inactive = np.unique(np.append(inactive, inactiveTest))
            #inactive = inactiveTest
            data[i, inactive] = 0
            inverse_data.append(indexed_string.inverse_removing(inactive))
